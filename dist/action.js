...
import { CheckoutBranchCommand, CommitChangesCommand, CreatePullRequestCommand, FetchLatestCommand, PushChangesCommand, StageChangesCommand, UpdateConfigCommand } from './commands';
...
export class Action {
    ...
    gitHub;
    commands = [];
    constructor(inputs, git = new Git(), gitHub) {
        this.git = git;
        this.gitHub = gitHub;
        ...
        const gitHubClient = this.gitHub ||
            new GitHubClient({
        ...
        this.commands.push(new UpdateConfigCommand(this.git, params.authorName, params.authorEmail, params.signCommit));
        if (params.fetchLatest) {
            this.commands.push(new FetchLatestCommand(this.git));
        }
        this.commands.push(new CheckoutBranchCommand(this.git, params.branch, params.createBranch));
        this.commands.push(new StageChangesCommand(this.git, params.directoryPath));
        const commitCommand = new CommitChangesCommand(this.git, params.commitMessage, params.signCommit);
        this.commands.push(commitCommand);
        this.commands.push(new PushChangesCommand(this.git, params.remoteRef, params.branch, params.forcePush));
        if (params.openPullRequest) {
            this.commands.push(new CreatePullRequestCommand(gitHubClient, params.branch));
        }
    }
    async execute() {
        try {
            ...
            const isNoChanges = error instanceof NoChangesError ||
            ...
            core.setFailed(`Action failed: ${message}. Please review the logs for more details.`);
        }
    }
}
