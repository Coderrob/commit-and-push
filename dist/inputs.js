...
export const actionInputs = {
    ...
    [Input.FETCH_LATEST]: {
        ...
        description: 'Whether to fetch the latest changes from the remote repository before pushing the commit',
        default: 'false',
        required: false,
        deprecationMessage: ''
    },
...
export const getInputValue = new Proxy({}, {
    get: (_, key) => {
        if (!Object.values(Input).includes(key)) {
            throw new Error(`Invalid input key: ${key}`);
        }
        const entry = actionInputs[key];
