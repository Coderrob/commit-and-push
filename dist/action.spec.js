...
describe('Action', () => {
    ...
    describe('constructor', () => {
        it('should throw error for invalid repository format', () => {
            ...
            expect(() => new Action(invalidInputs)).toThrow('Invalid repository format. Expected format: owner/repo');
        });
    ...
    describe('execute', () => {
        ...
        let updateConfigSpy;
        let fetchLatestSpy;
        let checkoutBranchSpy;
        let stageChangesSpy;
        let commitChangesSpy;
        let pushChangesSpy;
        beforeEach(() => {
        ...
        it('should execute all steps', async () => {
            ...
            expect(updateConfigSpy).toHaveBeenNthCalledWith(1, 'Captain Picard', 'jedi@example.com', true);
            ...
            expect(commitChangesSpy).toHaveBeenNthCalledWith(1, 'You shall not pass!', true);
        ...
        it('should skip push when no changes to commit', async () => {
            ...
            pushChangesSpy.mockReturnValue(Promise.resolve(0));
            await action.execute();
            expect(infoSpy).toHaveBeenCalledWith('No changes to commit. Skipping push and pull request.');
        ...
        it('should throw error when commit fails', async () => {
            ...
            await action.execute();
            expect(setFailedSpy).toHaveBeenCalledWith('Action failed: Commit failed. Please check your commit message format and ensure GPG is set up if commit signing is enabled.. Please review the logs for more details.');
        });
    });
});
