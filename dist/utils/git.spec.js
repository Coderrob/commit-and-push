...
describe('Git', () => {
    ...
    describe('updateConfig', () => {
        it('should update git config with name, email, and to sign commits', async () => {
            execCommandSpy
                ...
                .mockResolvedValueOnce({ exitCode: 0 })
                .mockResolvedValueOnce({ exitCode: 0 });
            const exitCode = await git.updateConfig('Captain Picard', 'jedi@example.com', true);
        ...
        it('should update git config with name and email', async () => {
            execCommandSpy
                ...
                .mockResolvedValueOnce({ exitCode: 0 })
                .mockResolvedValueOnce({ exitCode: 0 });
            const exitCode = await git.updateConfig('Captain Picard', 'jedi@example.com', false);
            expect(exitCode).toEqual(0);
    ...
    describe('stageChanges', () => {
        ...
        it('should throw error if directory does not exist', async () => {
            ...
            await expect(git.stageChanges('/nonexistent')).rejects.toThrow("Directory path '/nonexistent' does not exist.");
            expect(execCommandSpy).not.toHaveBeenCalled();
        });
    });
    describe('commitChanges', () => {
        it('should commit changes with the specified message and sign them if required', async () => {
            execCommandSpy.mockImplementationOnce(() => Promise.resolve({ exitCode: 0 }));
        ...
        it('should return 1 when no changes to commit', async () => {
            execCommandSpy.mockRejectedValueOnce(new Error('nothing to commit, working tree clean'));
        ...
        it('should return 2 when commit fails for other reasons', async () => {
            execCommandSpy.mockRejectedValueOnce(new Error('Some other commit error'));
