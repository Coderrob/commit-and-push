...
describe('Common', () => {
    ...
    describe('ensureQuoted', () => {
        it.each([
            ...
            ['hello\nworld', '"hello\nworld"']
        ])('should ensure that %s is quoted as %s', (input, expected) => expect(ensureQuoted(input)).toBe(expected));
    });
    describe('sanitizeInput', () => {
        it('should throw an error for non-string input', () => {
            expect(() => sanitizeInput(123)).toThrow('Invalid input type');
        });
        ...
        it.each([
            ...
            ['some $extra', /\$/g]
        ])('should throw an error if the input %s contains disallowed patterns %s', (input) => expect(() => sanitizeInput(input)).toThrow(`Security risk detected in input: ${input}`));
    });
    describe('executeGitCommand', () => {
        ...
        it('should throw an error for getExecOutput failure', async () => {
            ...
            await expect(() => execCommand({ command: GitCommand.PUSH })).rejects.toThrow(`Git command failed: Overdue electric bill`);
